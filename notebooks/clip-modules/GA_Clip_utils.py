...
def get_image(bikes, dataset, thread_count, num_views=10, timeout=10):
    ...
    #     except:
    #         pass
    processGen.processGen(bikes, dataset=dataset, from_OH=True, check=True, sourcepath="PlainRoadbikestandardized.txt",
                          targetpath="BCAD_gen/")
...
def get_clip_embedding_views(bikes, dataset, processor, model, thread_count, timeout=10, num_views=0, batchsize=200,
                             return_images=False):
...
def get_mean_embedding(bikes, dataset, processor, model, thread_count, timeout=10, num_views=0):
    valid_idxs, views_emb = get_clip_embedding_views(bikes, dataset, processor, model, thread_count, timeout=timeout,
                                                     num_views=num_views, return_images=False)
    if valid_idxs == []:
