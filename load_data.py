...
def load_scaled_framed_dataset():
    ALL_STRUCTURAL_DATASET = os.path.join(os.path.dirname(__file__), "FRAMED Dataset/all_structural_data_aug.csv")
...
def load_framed_dataset(c_r="c", onehot = True, scaled = True, augmented = False):
    ...
    if augmented:
        reg_data = pd.read_csv("FRAMED Dataset/all_structural_data_aug.csv", index_col=0)
        clf_data = pd.read_csv("FRAMED Dataset/validity_aug.csv", index_col=0)
    else:
        reg_data = pd.read_csv("FRAMED Dataset/all_structural_data.csv", index_col=0)
        clf_data = pd.read_csv("FRAMED Dataset/validity.csv", index_col=0)
